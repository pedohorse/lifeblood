...
class NodeSerializerV2(NodeSerializerBase):
    ...
    def serialize(self, node: BaseNode) -> bytes:
        ...
        return json.dumps(data_dict, cls=NodeSerializerV2.Serializer).encode('latin1')
    ...
    def deserialize(cls, parent: NodeGraphHolderBase, node_id: int, node_data_provider: NodeDataProvider, data: bytes) -> BaseNode:
        ...
        for musthave in ('format_version', 'type_name', 'type_definition_hash', 'state', 'parameters', 'name', 'ingraph_id'):
