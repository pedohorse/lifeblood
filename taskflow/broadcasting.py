...
async def create_broadcaster(identifier, information, broad_port=9000):
    loop = asyncio.get_event_loop()
    return await loop.create_datagram_endpoint(
        lambda: BroadcastProtocol(identifier, information, ('0.0.0.0', broad_port), loop=loop),
...
async def await_broadcast(identifier, broad_port=9000):
    ...
    _, protocol = await loop.create_datagram_endpoint(
        ...
        local_addr=('0.0.0.0', broad_port), reuse_port=True)
    return await protocol.till_done()


class BroadcastProtocol(asyncio.DatagramProtocol):
    def __init__(self, identifier: str, information: str, target: Address, *, loop: asyncio.AbstractEventLoop = None):
