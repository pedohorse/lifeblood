...
class TelegramClient:
    ...
    def send_media(self, chat_id: str, media_path: Union[Path, str], caption: str = ''):
        if isinstance(media_path, str):
            media_path = Path(media_path)

        if media_path.suffix in ('.gif',):
            return self.send_animation(chat_id, media_path, caption)
        elif media_path.suffix in ('.mp4',):
            return self.send_video(chat_id, media_path, caption)
        elif media_path.suffix in ('.bmp', '.jpg', '.jpeg', '.png',):
            return self.send_image(chat_id, media_path, caption)
        else:
            return self.send_document(chat_id, media_path, caption)

    def send_image(self, chat_id: str, image_path: Union[Path, str], caption: str = ''):
    ...
    def send_video(self, chat_id: str, video_path: Union[Path, str], caption: str = ''):
    ...
    def send_animation(self, chat_id: str, video_path: Union[Path, str], caption: str = ''):
        if isinstance(video_path, str):
            video_path = Path(video_path)
    ...
    def send_document(self, chat_id: str, file_path: Union[Path, str], caption: str = ''):
