...
def printwait(stuff, wait_scale=1.0):
    time.sleep(rng.uniform(0.05, 0.5)*wait_scale)
    print(stuff)
    time.sleep(rng.uniform(0.05, 0.5)*wait_scale)


def mock_render(rs_file: str, do_crap_myself=False):
    ...
    for line in suffix.format(blocks=blocks,
                              ...
                              out_beauty='/tmp/test/out.1234.exr',
                              out_additional='Saving: /tmp/fofo/with spaces/file.aov.1234.exr\n'
                                             'Saving: /tmp/fofo/with юникод щит/file.aov.1234.exr').splitlines():
        printwait(line, wait_scale=0.01)
