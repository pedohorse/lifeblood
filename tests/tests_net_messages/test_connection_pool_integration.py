...
from lifeblood.net_messages.tcp_impl.message_protocol import MessageProtocol
    ...
    async def test_pooled_connection_closed_from_another_side_real(self):
        ...
        async def _aconnsink():
            ...
            def _create_protocol():
                nonlocal prt
                prt = MessageProtocolTest((get_localhost(), 29360), _noop)
                return prt
        ...
        try:
            stream_stash = []
            timeout = 2
            pooled_factory = TcpMessageStreamPooledFactory(timeout, _fake_conn_opener_factory(stream_stash, _initialize_connection))
