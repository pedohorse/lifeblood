...
from taskflow.scheduler_worker_pool import SchedulerWorkerPool, create_scheduler_worker_pool


class WorkerPoolTests(TestCase):
    def __init__(self, method='runTest'):
        super(WorkerPoolTests, self).__init__(method)
        get_logger(SchedulerWorkerPool.__name__.lower()).setLevel('DEBUG')

    async def _helper_test_basic(self, rnd):
        swp = await create_scheduler_worker_pool()
    ...
    async def _helper_test_min1(self, rnd):
        mint = 4
        mini = 1
        swp = await create_scheduler_worker_pool(minimal_total_to_ensure=mint, minimal_idle_to_ensure=mini)
